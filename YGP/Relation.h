 ...
 */
template <class _Tp> struct lessDereferenced {
    ...
    bool operator()(const _Tp& __x, const _Tp& __y) const { return __x.get() < __y.get(); }
};
  ...
  public:
    ...
    void relate(const S& source, const T& target) {
        ...
        if (i == objects.end()) {
            ...
            for (typename std::map<S, T>::const_iterator i(objects.begin()); i != objects.end(); ++i)
    ...
    void unrelateAll() { objects.erase(objects.begin(), objects.end()); }
    ...
    S& getParent(const T& object) {
        Check1(object);
        for (typename std::map<S, T>::iterator i(objects.begin()); i != objects.end(); ++i)
            if (i->second == object)
                return i->first;
 ...
 */
template <class S, class T> class Relation1_N : public IRelation {
  public:
    ...
    void relate(const S& source, const T& target) {
        ...
        Check1(target);
#if defined(CHECK) && (CHECK > 0)
        for (typename std::map<S, std::vector<T>>::const_iterator i(objects.begin()); i != objects.end(); ++i)
            Check(find(i->second.begin(), i->second.end(), target) == i->second.end());
#endif
    ...
    void relate(const S& source, const std::vector<T>& targets) {
        ...
        for (typename std::map<S, std::vector<T>>::const_iterator i(objects.begin()); i != objects.end(); ++i)
            for (typename std::vector<T>::const_iterator t(targets.begin()); t != targets.end(); ++t)
                Check(find(i->second.begin(), i->second.end(), *t) == i->second.end());
#endif
    ...
    bool isRelated(const T& object) const {
        Check1(object);
        for (typename std::map<S, std::vector<T>>::const_iterator i(objects.begin()); i != objects.end(); ++i)
            if (find(i->second.begin(), i->second.end(), object) != i->second.end())
                return true;
        return false;
    }
    /// Checks if the passed object is related to any other (within this relation)
    ...
    bool isRelated(const S& owner, const T& object) const {
        ...
        return ((i != objects.end()) ? (find(i->second.begin(), i->second.end(), object) != i->second.end()) : false);
    }
    ...
    S getParent(const T& object) {
        Check1(object);
        for (typename std::map<S, std::vector<T>>::const_iterator i(objects.begin()); i != objects.end(); ++i) {
            typename std::vector<T>::const_iterator o(find(i->second.begin(), i->second.end(), object));
            if (o != i->second.end())
                return i->first;
        }
...
/**1-to-N relation with a limited N.
   ...
   Each object can be connected to various others.
 */
template <class S, class T> class Relation1_X : public Relation1_N<S, T> {
  public:
    /// Creates an 1-to-n relation.
    /// \param name Name of relation
    ...
    void relate(S source, T target) {
        Check1(source);
        Check1(target);
        typename std::map<S, std::vector<T>>::iterator i(Relation1_N<S, T>::objects.find(source));
 ...
 */
template <class S, class T> class RelationN_M : public IRelation {
  public:
    /// Creates an 1-to-n relation.
    /// \param name Name of relation
    ...
    const std::vector<S>& getParents(const T& object) const {
        ...
        for (typename std::map<S, std::vector<T>>::const_iterator i(objects.begin()); i != objects.end(); ++i) {
            typename std::vector<T>::const_iterator o(find(i->second.begin(), i->second.end(), object));
            if (o != i->second.end()) {
                typename std::map<T, std::vector<S>>::const_iterator p(parents.find(*o));
