...
// CREATED     : 16.7.1999
// COPYRIGHT   : Copyright (C) 1999 - 2004, 2008, 2026
...
int extStreambuf::underflow() {
    ...
    TRACE7("extStreambuf::underflow() - Act: " << std::hex << gptr() << "; End: " << egptr() << std::dec);
...
//-----------------------------------------------------------------------------
std::streampos extStreambuf::seekoff(std::streamoff off, std::ios_base::seekdir dir, std::ios_base::openmode mode) {
    TRACE7("extStreambuf::seekoff(streamoff, _seek_dir, mode) - " << off << "; " << dir << '/' << mode);
    Check1(pSource);
...
//-----------------------------------------------------------------------------
int extStreambuf::checkIntegrity() const { return pBuffer ? pSource ? 0 : 1 : 2; }
