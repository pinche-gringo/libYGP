...
struct File {
    ...
    File() : path_(), entry(), status(), userExec(false) { *entry.d_name = '\0'; }
    ...
    void time(struct tm& time) const { time = *gmtime(&status.st_mtime); }
    /// Retrieving the time the file was modified the last time (local time)
    void localtime(struct tm& time) const { time = *::localtime(&status.st_mtime); }
...
struct File : protected WIN32_FIND_DATA {
    ...
    time_t time() const;
    void time(struct tm& time) const { setTime(ftLastWriteTime, time); }
  ...
  protected:
    ...
    void name(const char* name) {
        if (name)
            strncpy(cFileName, name, sizeof(cFileName));
    }
    void name(const std::string& name) {
