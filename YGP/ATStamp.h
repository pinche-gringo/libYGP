...
class ATimestamp : virtual public ADate, virtual public ATime {
  public:
    ...
    ATimestamp(const ATimestamp& other)
        : ADate((const ADate&)other), ATime((const ATime&)other) {} ///< Copy constructor from another ATimestamp object
    ATimestamp(char Day, char Month, int Year, char Hour, char minute, char second);
    ATimestamp(const char* pStamp) { operator=(pStamp); }      ///< Constructor from an (unformatted) text
    ATimestamp(const std::string& stamp) { operator=(stamp); } ///< Constructor from an (unformatted) text
    ATimestamp(const struct tm& tm) { operator=(tm); }         ///< Constructor from a broken down time
