...
class ATime : public AttributValue {
  public:
    ...
    ATime(const ATime& other) /// Copy constructor from another time object
        : AttributValue((const AttributValue&)other), hour(other.hour), min_(other.min_), sec(other.sec), mode(other.mode) {}
    ATime(char Hour, char minute, char second);
    ATime(const char* pTime) : AttributValue(), hour(0), min_(0), sec(0), mode(MODE_LOCALE) {
        operator=(pTime);
    } ///< Constructor from a text (unformatted)
    ATime(const std::string& time) : AttributValue(), hour(0), min_(0), sec(0), mode(MODE_LOCALE) {
        operator=(time);
    } ///< Constructor from a text (unformatted)
    ...
    ATime& operator=(const struct tm& tm) { /// Assignment operator from broken down time
        ...
        min_ = (unsigned char)tm.tm_min;
        setSecond((unsigned char)tm.tm_sec);
        return *this;
    }
