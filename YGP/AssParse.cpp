...
// CREATED     : 25.8.2001
// COPYRIGHT   : Copyright (C) 2001 - 2010, 2026
...
std::string AssignmentParse::getNextNode() {
    ...
    spirit::rule<> value(
        spirit::confix_p(QUOTE, (*spirit::c_escape_ch_p)[spirit::assign_a(actValue)], QUOTE)[spirit::assign_a(quoted, true)] |
        (+(~spirit::ch_p(SEPARATOR)))[spirit::assign_a(actValue)]);
    spirit::rule<> assignment = ((key[spirit::assign_a(actKey)] >> '=' >> value)[spirit::assign_a(node)] >>
    ...
    if (quoted)
        for (std::string::size_type pos(0); (pos = actValue.find("\\\"", pos)) != std::string::npos; ++pos)
...
//-----------------------------------------------------------------------------
std::string AssignmentParse::makeAssignment(const char* key, const char* value, size_t length) {
    TRACE9("AssignmentParse::makeAssignment(const char*, const char*) - " << key << " = " << value);
    Check1(key);
    Check1(value);
...
std::string AssignmentParse::makeAssignment(const char* key, const std::string& value) {
    TRACE9("AssignmentParse::makeAssignment(const char*, const std::string&) - " << key << " = " << value);
    Check1(key);
