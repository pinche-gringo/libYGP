//$Id: Process.cpp,v 1.1 2003/02/05 02:03:00 markus Exp $
...
//            not be found or produces an error while executing an describing
//            text is returned.
...
/*--------------------------------------------------------------------------*/
void Process::execAsync (const char* file, const char* const arguments[])
   throw (std::string)
{
   string error;
   int pipes[2];
   ...
   default: {
      ...
      sleep (1);
      int rc (0), wait;
      if ((wait = waitpid (pid, &rc, WNOHANG)) < 0)
         break;
