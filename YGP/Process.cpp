...
// CREATED     : 04.02.2003
// COPYRIGHT   : Copyright (C) 2003 - 2006, 2008, 2026
...
//-----------------------------------------------------------------------------
pid_t Process::start(const char* file, const char* const arguments[], int flags, int* fd) {
    ...
    if ((pipe(pipes) != -1) && ((flags & CONNECT_STDOUT_AND_ERR) && pipe(errPipes) != -1)) {
        ...
        if ((sout != -1) && (serr != -1)) {
            ...
            pid = spawnvp((flags & WAIT) ? P_WAIT : P_NOWAIT, file, const_cast<char* const*>(arguments));
    ...
    if (((pid = pipe(pipes)) != -1) && ((flags & CONNECT_STDERR) ? ((pid = pipe(errPipes)) != -1) : true)) {
        pid = fork();
...
std::string Process::readChildOutput(int file) {
    ...
    char buffer[80];
    int cChar;
    while ((cChar = read(file, buffer, sizeof(buffer))) && (cChar != -1))
