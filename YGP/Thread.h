...
class Thread {
  public:
    ...
    static Thread* create(THREAD_FUNCTION fnc, void* pArgs) { return new Thread(fnc, pArgs); }
...
#    endif
  ...
  protected:
    Thread();
    Thread(THREAD_FUNCTION fnc, void* pArgs);

    void ret(void* rc) const;
    void init(THREAD_FUNCTION fnc, void* pArgs);
...
template <class T> class OThread : public Thread {
  public:
    ...
    /// \param pArgs Argument to the thread
    static OThread<T>* create(T* obj, THREAD_OBJMEMBER fnc, void* pArgs) { return new OThread<T>(obj, fnc, pArgs); }
    /// Creates a new thread; a pointer to the thread is passed to the thread function
    /// \param obj Object having a member to execute in a thread
    /// \param fnc Member to execute as thread
    /// \param pArgs Argument to the thread
    static OThread<T>* create2(T* obj, THREAD_OBJMEMBER fnc, void* pArgs) { return new OThread<T>(obj, fnc, pArgs, true); }
