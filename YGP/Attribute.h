...
class IAttribute {
  public:
    ...
    /// Checks if the passed text matches the name of the attribute
    bool matches(const std::string& name_) const { return name == name_; }
...
template <> inline bool Attribute<bool>::assignFromString(const char* value) const {
    Check3(value);
    if (!(strcmp(value, "1") && strcmp(value, "true") && strcmp(value, "True") && strcmp(value, "TRUE")))
        return attr_ = true;
    if (!(strcmp(value, "0") && strcmp(value, "false") && strcmp(value, "False") && strcmp(value, "FALSE"))) {
        attr_ = false;
        return true;
    }
    return false;
}
template <> inline std::string Attribute<bool>::getValue() const { return attr_ ? "0" : "1"; }
template <> inline std::string Attribute<bool>::getFormattedValue() const { return attr_ ? "True" : "False"; }
...
template <class T, class L = std::vector<T>> class AttributeList : public IAttribute {
  public:
    ...
    bool isValidOffset(unsigned int offset) const { return offset <= list_.size(); }
...
template <> inline bool AttributeList<char>::doAssignFromString(unsigned int offset, const char* value) const {
    Check3(value);
    list_.at(offset) = *value;
    return *value && !value[1];
}
...
/// \returns \c true on success; \c false otherwise
template <> inline bool AttributeList<std::string>::doAssignFromString(unsigned int offset, const char* value) const {
    Check3(value);
    list_.at(offset) = value;
    return true;
}
...
/// \returns \c true on success; \c false otherwise
template <>
inline bool AttributeMap<std::string>::assign(const std::string& offset, const char* value, unsigned int length) const {
    Check3(value);
