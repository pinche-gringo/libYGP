...
#define INISECTION(section)                                                                                                      \
    YGP::INISection section(#section);                                                                                           \
    _inifile_.addSection(section);
#define INIATTR(section, type, attr)                                                                                             \
    YGP::Attribute<type> section##attr##_(#attr, (attr));                                                                        \
    (section).addAttribute(section##attr##_);
#define INIATTR2(section, type, attr, name)                                                                                      \
    YGP::Attribute<type> section##name##_(#name, (attr));                                                                        \
    (section).addAttribute(section##name##_);
#define INIATTR3(section, metaenum, attr)                                                                                        \
    YGP::MetaEnumAttribute section##attr##_(#attr, (metaenum), (attr));                                                          \
    (section).addAttribute(section##attr##_);
#define INIATTR4(section, metaenum, attr, name)                                                                                  \
    YGP::MetaEnumAttribute section##name##_(#name, (metaenum), (attr));                                                          \
    (section).addAttribute(section##name##_);
#define INILIST(name, type)                                                                                                      \
    YGP::INIList<type> name(#name, name);                                                                                        \
    _inifile_.addSection(name);
#define INILIST2(section, type, name)                                                                                            \
    YGP::INIList<type> section(#section, name);                                                                                  \
    _inifile_.addSection(section);
#define INIMAP(name, type)                                                                                                       \
    YGP::INIMap<type> name(#name, name);                                                                                         \
    _inifile_.addSection(name);
#define INIMAP2(section, type, name)                                                                                             \
    YGP::INIMap<type> section(#section, name);                                                                                   \
    _inifile_.addSection(section);
...
class INISection {
  ...
  public:
    ...
    void writeHeader(std::ostream& stream) { writeHeader(stream, pName); }
    /// Writes the passed name as header of a section to the passed stream.
    static void writeHeader(std::ostream& stream, const char* section) { stream << '[' << section << "]\n"; }
...
template <class T, class L = std::vector<T>> class INIList : public INISection {
  public:
    ...
    void write(std::ostream& stream) {
        return write(stream, attributes[0]->getName(), ((AttributeList<T>*)attributes[0])->getAttribute());
    }
  ...
  protected:
    ...
    int foundKey(const char* key, unsigned int) override {
        ...
        return ((errno || *pEnd || !((AttributeList<T, L>*)pFoundAttr)->isValidOffset(offset)) ? ParseObject::PARSE_CB_ABORT
...
template <class T, class M = std::map<std::string, T>> class INIMap : public INISection {
  public:
    ...
    /// \param stream Stream to write to
    void write(std::ostream& stream) {
        return write(stream, attributes[0]->getName(), ((AttributeMap<T>*)attributes[0])->getAttribute());
    }
    /// Writes the contents of the passed values to the passed stream
    /// (in its own section named \c section).
    /// \param stream Stream to write to
...
class INIFile {
  public:
    ...
    /// Writes a header for the section to the passed stream.
    static void writeSectionHeader(std::ostream& stream, const char* section) { INISection::writeHeader(stream, section); }
