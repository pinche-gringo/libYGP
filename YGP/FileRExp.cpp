...
static inline bool isRegionNegator(const char ch) { return (ch == NEGREGION1) || (ch == NEGREGION2); }
#else

static inline bool isRegionNegator(const char ch) { return ch == NEGREGION1; }
#endif
            ...
            while ((ch = *++pAktRegExp) != REGIONEND) {
                if (pAktRegExp[1] == RANGE) {
                    ...
                    if ((ch == REGIONBEGIN) && (pAktRegExp[1] == REGIONCLASS) &&
                        ((pEndClass = strchr(pAktRegExp + 2, REGIONCLASS)) != NULL) && (pEndClass[1] == REGIONEND)) {
                        TRACE7("Check " << *pCompare << " against region-class " << pAktRegExp + 2);
                        inClass = true;
                        ...
                        int val((temp = isclass(alnum, pAktRegExp + 2, len, *pCompare))   ? temp
                                ...
                                : (temp = isclass(lower, pAktRegExp + 2, len, *pCompare))
                                    ? temp
                                    : (isclass(xdigit, pAktRegExp + 2, len, *pCompare)));
                        if (val == 2) {
                            TRACE8("Check " << *pCompare << " matches region-class " << pAktRegExp + 2);
                            break;
                        } // endif class and input matches
                        else {
                            TRACE8("Check " << *pCompare << " doesn't match region-class " << pAktRegExp << " -> RC = " << val);
                            if (val)
            ...
            while (ch != REGIONEND) {
                if (ch == REGIONCLASS) {
                    if (pAktRegExp[-1] == REGIONBEGIN)
                        inClass = true;
                    else if (pAktRegExp[1] == REGIONEND)
