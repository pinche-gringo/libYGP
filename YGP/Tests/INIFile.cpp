...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        try {
            ...
            throw(std::string("Error parsing INIFile.test in line ") + YGP::ANumeric(_inifile_.getFile().getLine()).toString() +
                  std::string(" (") + YGP::ANumeric(_inifile_.getFile().getColumn()).toString() + std::string("): ") + e.what());
        } // end-catch
