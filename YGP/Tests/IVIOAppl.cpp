...
class Application : public YGP::IVIOApplication {
  public:
    Application(const int argc, const char* argv[]) : YGP::IVIOApplication(argc, argv, lo), cOptions(0), cErrors(0) {}
    ~Application() {}
...
const YGP::IVIOApplication::longOptions Application::lo[] = {
    {"help", 'h'}, {"stackdump", 'd'}, {"arg-opt", 'a'}, {"arg-mand", 'A'}, {NULL, '\0'}};
...
int Application::perform(int argc, const char* argv[]) {
    ...
    std::cout << "    " << cOptions << " options and " << argc << " arguments passed\n";
    check(!argv[argc]);
