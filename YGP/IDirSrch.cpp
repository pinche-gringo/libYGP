...
// CREATED     : 22.8.2001
// COPYRIGHT   : Copyright (C) 2001 - 2004, 2008, 2026
...
static const unsigned int FILE_NORMAL_ =
    ((S_IFREG) | (S_IFLNK) | (S_ISUID) | (S_ISGID) | (S_ISVTX) | (S_IRWXU) | (S_IRWXG) | (S_IRWXO));
static const unsigned int FILE_READONLY_ = ((S_IFREG) | (S_IFLNK) | (S_ISUID) | (S_ISGID) | (S_ISVTX) | (S_IRUSR) | (S_IRGRP) |
                                            (S_IROTH) | (S_IXUSR) | (S_IXGRP) | (S_IXOTH));
static const unsigned int FILE_DIRECTORY_ = ((S_IFDIR) | (S_ISUID) | (S_ISGID) | (S_ISVTX) | (S_IRWXU) | (S_IRWXG) | (S_IRWXO));
...
#define ADDSYSATTRIB(result, sample, flag)                                                                                       \
    if (sample & IDirectorySearch::flag)                                                                                         \
        result |= flag##_;
...
/// Destructor
//-----------------------------------------------------------------------------
IDirectorySearch::~IDirectorySearch() { delete pEntry; }
...
unsigned long IDirectorySearch::convertToSysAttribs(unsigned long attribs) {
    TRACE3("IDirectorySearch::convertToSysAttribs(unsigned long) - " << std::hex << attribs);
    ...
    TRACE3("IDirectorySearch::convertToSysAttribs(unsigned long) - System: " << std::hex << attr);
    return attr;
}
...
unsigned long IDirectorySearch::convertFromSysAttribs(unsigned long attribs) {
    TRACE3("IDirectorySearch::convertFromSysAttribs(unsigned long) - " << std::hex << attribs);
    unsigned long attr = 0;
...
#    error Not implemented yet!
    ...
    TRACE3("IDirectorySearch::convertFromSysAttribs(unsigned long) - " << std::hex << attr);
    return attr;
}
