...
/// Destructor
//-----------------------------------------------------------------------------
DirectorySearch::~DirectorySearch() { cleanup(); }
...
const File* DirectorySearch::find(unsigned long attribs) {
    ...
    TRACE5("DirectorySearch::find(unsigned long) " << searchDir.c_str() << searchFile.c_str());

    pEntry = new File;
    // Set path-information; but cut path in case of special cases
    pEntry->path_ = offStrip ? searchDir.substr((offStrip > 0) ? offStrip : ((offStrip < 0) ? -offStrip : 0)) : searchDir;
...
#    if SYSTEM == WINDOWS
    ...
    return ((pEntry->attributes() & attr_) || !regExp.matches(pEntry->name())) ? next() : pEntry;
#    else
#        error Not implemented!
#    endif
#endif
    ...
    while ((pDirEnt = readdir(pDir)) != nullptr) { // Files available?
        ...
        if (regExp.matches(pDirEnt->d_name)) {
            ...
            if (!stat(temp.c_str(), &pEntry->status)) {
                // Do attributes match?
                TRACE9("DirectorySearch::next(): " << pDirEnt->d_name << " (" << std::hex << attr_ << ") -> Mode: "
		       << pEntry->status.st_mode << std::dec);
...
#    if SYSTEM == WINDOWS
    ...
    while (FindNextFile(hSearch, pEntry))
        if (!(pEntry->attributes() & attr_) && regExp.matches(pEntry->name())) {
...
int DirectorySearch::checkIntegrity() const {
    ...
    return (searchDir.empty()
                ? NO_DIR
                : (searchFile.empty() ? NO_FILE : (pEntry ? DIRSRCH_OK + (pEntry->path_.empty() && !offStrip) : NO_ENTRY)));
}
...
/// \returns bool True if the directory exists
//-----------------------------------------------------------------------------
bool DirectorySearch::isValid() const { return isValid(searchDir); }
