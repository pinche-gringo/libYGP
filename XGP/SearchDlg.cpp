...
/// (Default-)Constructor
//-----------------------------------------------------------------------------
SearchDialog::SearchDialog() : XGP::XDialog(OKCANCEL), signalFind(), find(Gtk::make_managed<Gtk::Entry>()) {
    set_title(_("Search"));
...
//-----------------------------------------------------------------------------
void SearchDialog::okEvent() { signalFind.emit(last = find->get_text()); }

//-----------------------------------------------------------------------------
/// Callback after changing the text to find
//-----------------------------------------------------------------------------
void SearchDialog::inputChanged() { ok->set_sensitive(find->get_text_length()); }
