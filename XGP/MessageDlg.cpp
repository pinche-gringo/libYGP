...
MessageDlg::MessageDlg(const YGP::StatusObject& obj)
    : Gtk::MessageDialog(obj.getMessage(), false, getButtonType(obj.getType())), detail(Gtk::make_managed<Gtk::Label>()),
...
Gtk::MessageType MessageDlg::getButtonType(YGP::StatusObject::type tp) {
    return ((tp == YGP::StatusObject::ERROR) ? Gtk::MessageType::ERROR : static_cast<Gtk::MessageType>(tp));
}
...
void MessageDlg::showDetails(bool show) {
    ...
    Glib::signal_idle().connect(sigc::bind(sigc::mem_fun(*this, &MessageDlg::doRegister), !show));
}
...
bool MessageDlg::doRegister(bool show) {
    cb.disconnect();
    cb = showDetail->signal_clicked().connect(sigc::bind(sigc::mem_fun(*this, &MessageDlg::showDetails), show));
    return false;
}
...
//-----------------------------------------------------------------------------
void MessageDlg::free(int) { delete this; }
