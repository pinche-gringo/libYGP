...
void XFileList::loadIcons(const char* path, const char* files, unsigned int namePrefix) {
    ...
    if (!iconExe) {
        ...
        filename += file->name();
        iconDir = Gdk::Pixbuf::create_from_file(filename);
        Check1(iconDir);

        file = ds.find("Default.png");
        Check2(file);
        filename = file->path();
        filename += file->name();
        iconDef = Gdk::Pixbuf::create_from_file(filename);

        file = ds.find("Executable.png");
        Check2(file);
        filename = file->path();
        filename += file->name();
        ...
        while (file) {
            ...
            type.assign(pTypepart, strrchr(pTypepart, '.') - pTypepart);
    ...
    if (file.isDirectory())
        ...
        do {
            ...
            if ((i = icons.find(++pName)) != icons.end()) {
                TRACE9("XFileList_setIcon(Gtk::TreeModel::iterator, const File&) - Use icon " << pName << " for file " << file.name());
                actIcon = (*i).second;
                break;
...
void XFileList::onRightClick(int, double x, double y) {
    ...
    if (get_path_at_pos(static_cast<int>(x), static_cast<int>(y), pathAct) && !pathAct.empty()) {
        TRACE8("XFileList::onRightClick(int, double, double) - Creating menu");

        // Get selected row
        Check3(get_model());
        Gtk::TreeModel::iterator iAct(get_model()->get_iter(pathAct));
        Check3(iAct);
        ...
        actions->add_action("open", sigc::bind(sigc::mem_fun(*this, &XFileList::startInTerm), ed, iAct));
        menu->append(editor, "popup.open");

        actions->add_action("move", sigc::bind(sigc::mem_fun(*this, &XFileList::move), iAct));
        menu->append(_("Rename/Move ..."), "popup.move");

        actions->add_action("delete", sigc::bind(sigc::mem_fun(*this, &XFileList::remove), iAct));
...
void XFileList::startInTerm(const char* file, Gtk::TreeModel::iterator line) {
    ...
    if (term) {
        ...
        Gtk::MessageDialog msg(_("Environment variable `TERM' not defined!"), false, Gtk::MessageType::ERROR);
        runModal(msg);
    }
}
...
void XFileList::move(Gtk::TreeModel::iterator line) {
    std::string file(
        FileDialog::create(std::string("Move file to ..."), Gtk::FileChooser::Action::OPEN, FileDialog::ASK_OVERWRITE)
            ->execModal());
...
//-----------------------------------------------------------------------------
void XFileList::addMenus(const Glib::RefPtr<Gio::Menu>&, const Glib::RefPtr<Gio::SimpleActionGroup>&,
